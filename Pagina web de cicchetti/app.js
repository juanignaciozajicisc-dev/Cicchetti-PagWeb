...
const ADMIN_KEY_STORAGE = "cicchetti.adminKey.v1";
...
const unlockAdminButton = document.querySelector("#unlockAdmin");
...
async function loadReservations() {
  ...
  if (!response.ok) {
    if (response.status === 401) {
      throw new Error("Ingresá la clave del local para ver las reservas.");
    }
...
function bookedGuestsForSlot(dateValue, timeValue) {
  if (API_MODE) {
    const slot = availabilityCache.find(
      (reservation) => reservation.date === dateValue && reservation.time === timeValue
    );
    return slot ? Number(slot.booked) : 0;
  }
...
async function renderReservations() {
  ...
  if (API_MODE) {
    try {
      ...
      if (reservationsBody) {
        reservationsBody.innerHTML = `<tr><td class="empty-row" colspan="7">${escapeHTML(error.message)}</td></tr>`;
      }
      source = [];
      return;
    }
...
async function exportCSV() {
  if (API_MODE) {
    try {
      const response = await fetch("/api/reservations.csv", {
        headers: {
          "X-Admin-Key": adminKey,
        },
      });
